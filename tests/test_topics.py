...
class TestTopicEvent:
    ...
    def test_round_trip(self):
        e = TopicEvent(event_type="progress", description="did stuff", actor="user")
        d = e.to_dict()
        restored = TopicEvent.from_dict(d)
...
class TestTopic:
    def test_creation_defaults(self):
        t = Topic(title="Test")
        assert t.title == "Test"
        assert t.status == TopicStatus.PENDING
        assert t.done == []
        assert t.pending == []
        assert t.events == []
...
class TestStatusTransitions:
    def test_set_status(self):
        t = Topic(title="S")
        t.set_status(TopicStatus.IN_PROGRESS, note="starting")
        assert t.status == TopicStatus.IN_PROGRESS
    ...
    def test_set_status_without_note(self):
        t = Topic(title="S")
        t.set_status(TopicStatus.BLOCKED)
        assert t.status == TopicStatus.BLOCKED
        assert len(t.events) == 1

    def test_multiple_transitions(self):
        t = Topic(title="S")
        t.set_status(TopicStatus.IN_PROGRESS)
        t.set_status(TopicStatus.AWAITING_USER)
        t.set_status(TopicStatus.AWAITING_AGENT)
        t.set_status(TopicStatus.COMPLETED)
        assert t.status == TopicStatus.COMPLETED
...
class TestSummary:
    def test_summary_with_action(self):
        t = Topic(title="ModelLedger", current_action="renaming files")
        t.set_status(TopicStatus.IN_PROGRESS)
        s = t.summary()
...
class TestTopicManager:
    def test_create(self, tmp_path):
        tm = TopicManager(storage_dir=tmp_path)
        topic = tm.create("Test Topic", description="desc", tags=["code"])
    ...
    def test_update_status(self, tmp_path):
        tm = TopicManager(storage_dir=tmp_path)
        topic = tm.create("S")
        tm.update_status(topic.topic_id, TopicStatus.IN_PROGRESS, note="go")
    ...
    def test_require_raises(self, tmp_path):
        ...
        with pytest.raises(KeyError):
            tm.update_status("bad-id", TopicStatus.IN_PROGRESS)
...
class TestSnapshot:
    ...
    def test_snapshot_format(self, tmp_path):
        ...
        tm.update_status(t1.topic_id, TopicStatus.IN_PROGRESS)
        ...
        tm.update_status(t2.topic_id, TopicStatus.AWAITING_VERIFICATION)
...
class TestPersistence:
    ...
    def test_save_load_round_trip(self, tmp_path):
        ...
        assert loaded.status == TopicStatus.IN_PROGRESS
    ...
    def test_archive_and_reload(self, tmp_path):
        ...
        loaded = tm2.get(tid)
        assert loaded is not None
        assert loaded.status == TopicStatus.COMPLETED
